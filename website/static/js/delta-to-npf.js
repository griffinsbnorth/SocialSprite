 ...
 * @typedef {{start: number, end: number, type: string, url?: string, hex?: string, blog?: string}} NpfFormatting
 */

//const deltaToIntermediate = require("./delta-to-intermediate").default;
    ...
    for (const block of blocks) {
        ...
        for (const inline of block.children) {
            ...
            if (typeof inline.insert === 'string') {
                ...
                const first = inline.insert.substring(0, 1);

                if (first == '@' && inline.attributes.link) {
                    let blogname = inline.attributes.link.replace('@', '').trim();
                    formatting.push({ start, end, type: 'mention', blog: blogname });
                } else {
                    ...
                    if (inline.attributes.strike) {
                        formatting.push({ start, end, type: 'strikethrough' });
                    }
